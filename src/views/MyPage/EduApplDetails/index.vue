...
async function getEduApplList() {
  loadingContainer.value.classList.add('loading');
  try {
    const response = await eduParticipateAPI.getEduApplList(searchIndex.value.pageNo);
    responseData.value.pager = setPager(response.data.pager);
    const prgmList = response.data.eduApplList;
    responseData.value.programList.length = 0;
    for (let appl of prgmList) {
      let result = appl.eduProgram;
      let eduProgram = {
        no: result.programNo,
        center: findRegionWithRegionNo(result.recruitRegion, store.state.regionCode.regionList),
        rcCount: result.recruitCnt,
        title: result.programTitle,
        eduDate: dateFormat(result.actBgnDate) + ' - ' + dateFormat(result.actEndDate),
        url: `/Details/Participation/VolTraining/ViewEduProgramDetail?programNo=${result.programNo}`,
        applStts: appl.sttsNo,
        isExternal: false
      }
      responseData.value.programList.push(eduProgram);
    }
  } catch (error) {
    console.log(error);
  }
