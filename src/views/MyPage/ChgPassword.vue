<template>
  ...
  <div>
    ...
    <h6 class="mt-2">
      8자리 이상 15자리 미만으로, 영문, 숫자, 특수문자 1자 이상 필수 입력
      <span class="highlight"> (예:aabb1234#)</span>
    </h6>
    <div class="box">
      <div class="row align-items-center">
        <div class="col-4 text-end pe-3">기존 비밀번호</div>
        <div class="col">
          <input v-model="oldPassword" class="form-control form-control-sm custom-input" type="password" aria-label=".form-control-sm example" />
        </div>
      </div>
      <div class="row mt-3 align-items-center">
        <div class="col-4 text-end pe-3">새 비밀번호</div>
        <div class="col">
          <input v-model="newPassword" class="form-control form-control-sm custom-input" type="password" aria-label=".form-control-sm example" />
        </div>
      ...
      <div class="row mt-3 align-items-center">
        ...
        <div class="col">
          <input v-model="confirmPassword" class="form-control form-control-sm custom-input" type="password" aria-label=".form-control-sm example" />
        </div>
...
<script setup>
import { ref } from 'vue';
import memberAPI from '@/apis/memberAPI';  // 경로 확인
...
const changePassword = async () => {
  ...
  try {
    // 비밀번호 변경 API 호출
    const response = await memberAPI.changePw({
    ...
    if (response.data.result === 'success') {
      ...
      newPwErrorMsg.value = response.data.reason || '비밀번호 변경에 실패했습니다.';
    }
  } catch (error) {
    if (error.response) {
      newPwErrorMsg.value = error.response.data.reason || '서버 오류가 발생했습니다.';
    } else {
